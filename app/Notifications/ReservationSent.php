    ...
    public function toMail($notifiable)
    {
        ...
        if($tran->payment_id == 1) {
            ...
            return (new MailMessage)
                    ->subject("Reservation")
                    ->greeting("Hi ".$fname."!")
                    ->line("Thank you for booking. It's on-hold until we confirm that payment has been received. In the meantime, here's a reminder of what you ordered:")
                    ->line("Send the deposit slip through our Facebook Page's Messenger Account SAKA RESORT.")
                    ->line("Our bank details")
                    ->line("John Doe")
                    ->line("Bank: BDO")
                    ->line("Account number: 004358012183")
                    ->line("Control Number: ". $tran->id)
                    ->line("Check in date: ". Carbon::parse($tran->checkIn_at)->toDayDateTimeString())
                    ->line("Check out date: ". Carbon::parse($tran->checkOut_at)->toDayDateTimeString())
                    ->line($type)
